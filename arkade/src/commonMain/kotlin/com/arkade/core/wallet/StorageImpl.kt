 ...
 *
 * @return A list of `WalletEntity` objects representing all stored wallets; an empty list if no wallets exist.
 */
 ...
 *
 * @param wallet The wallet entity containing updated fields; its `id` is used to identify which record to update.
 */
override suspend fun updateWallet(wallet: WalletEntity) = walletDao.update(wallet)
    ...
    companion object {
        ...
        /**
             ...
             *
             * If `testDb` is non-null, returns a cached test Storage created with that Database; otherwise
             * returns a cached production Storage. Initialization is performed once and the instance is
             * reused for subsequent calls.
             *
             * @param testDb Optional test Database to create or retrieve a test-backed Storage instance.
             * @return The singleton Storage; a test-backed instance when `testDb` is provided, otherwise the production instance.
             */
         ...
         *
         * After calling this, the next `get(testDb = ...)` call will create a new test-backed `Storage`.
         */
        @VisibleForTesting
        fun reset() {
            testStorage = null
